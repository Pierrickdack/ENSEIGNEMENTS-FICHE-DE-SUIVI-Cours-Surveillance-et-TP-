            ...
            <?php
              ...
              try{
                ...
                if(isset($_POST['edit_btn'])){
                    $id=$_POST['edit_id'];
                    $request=$connexion -> prepare("SELECT * FROM `fiche`");
                    $request -> execute();
                    ...
                    for ($i=0; $i < count($resultat); $i++) { 
                      if($id==$resultat[$i][0]){
                        ...
                        $ue=$resultat[$i][13];
                        $prof=$resultat[$i][14];
                        $query1 =$connexion -> prepare("INSERT INTO corbeille(ID_prof,Titre_c,Heure_deb_c,Heure_fin_c,Heure_total_c,dat_c,semestre_c,salle_c,type_seance_c,certificat_c,signature_c,Matricule,ID_chef,ID_ue)VALUES('$prof','$titre','$Hd','$Hf','$Ht','$date','$sem','$salle','$typeSe','$cert','$signe','$mat','123456','$ue')");
                        $query1 -> execute();
      ...
      labels: [
        <?php
            ...
            try{
              $connexion=new PDO("mysql:host=$server;dbname=ict201",$login,$pass);
              $connexion -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $request=$connexion -> prepare("SELECT `code_ue`,`ID_ue` FROM `ue` ORDER BY `ID_ue` ASC");
              $request -> execute();
              $resultat=$request -> fetchall();
              $a=$resultat[0][0];
              echo "'$a'";
        ...
        data: [
          <?php
            ...
            try{
              $connexion=new PDO("mysql:host=$server;dbname=ict201",$login,$pass);
              $connexion -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $request=$connexion -> prepare("SELECT ID_ue FROM `ue` ORDER BY `ID_ue` ASC");
              $request -> execute();
