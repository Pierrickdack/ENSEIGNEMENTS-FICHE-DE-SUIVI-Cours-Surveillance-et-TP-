        ...
        <table class="table table-striped table-sm" id="list">
          ...
          <?php
            ...
            try{
              $connexion=new PDO("mysql:host=$server;dbname=ict201",$login,$pass);
              $connexion -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $request=$connexion -> prepare("SELECT ID_ue FROM `ue` WHERE niveau_ue='2'");
              $request -> execute();
      ...
      labels: [
        <?php
            ...
            try{
              $connexion=new PDO("mysql:host=$server;dbname=ict201",$login,$pass);
              $connexion -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $request=$connexion -> prepare("SELECT `code_ue`,`ID_ue` FROM `ue` WHERE niveau_ue='2' ORDER BY `ID_ue` ASC");
              $request -> execute();
              $resultat=$request -> fetchall();
              $a=$resultat[0][0];
              echo "'$a'";
        ...
        data: [
          <?php
            ...
            try{
              $connexion=new PDO("mysql:host=$server;dbname=ict201",$login,$pass);
              $connexion -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $request=$connexion -> prepare("SELECT ID_ue FROM `ue` WHERE niveau_ue='2' ORDER BY `ID_ue` ASC");
              $request -> execute();
              $resultat1=$request -> fetchall();
