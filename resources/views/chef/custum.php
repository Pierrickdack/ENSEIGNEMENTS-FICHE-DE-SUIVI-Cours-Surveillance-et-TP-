        ...
        <div class="container col-xl-10 col-xxl-8 px-4 py-5">
            <div class="row align-items-center g-lg-5 py-5">
              <div class="col-lg-7 text-center text-lg-start">
                ...
                <p class="col-lg-10 fs-4">Cette section est reserve uniquement au chef et à la gestion des utilisateurs (delegués) notament la suppression et l'ajout. Les informations entrées devront etre remissent au délégué en question.</p>
              </div>
              <div class="col-md-10 mx-auto col-lg-5">
                <form class="p-4 p-md-5 border rounded-3 bg-light" id="addForm" method="post">
                  <div class="form-floating mb-3">
                    <input type="text" class="form-control" id="floatingMat" name="id" required>
                    <label for="floatingInput">Matricule / ID</label>
                  </div>
                  <div class="form-floating mb-3">
                  ...
                  <div class="form-floating mb-3">
                    <input type="password" class="form-control" id="floatingPassword" name="pas" required>
          ...
          <?php
              ...
              try{
                ...
                if(isset($_POST['edit_btn'])){
                    $id=$_POST['edit_id'];
                    $request=$connexion -> prepare("SELECT Matricule FROM `delegue`");
                    $request -> execute();
                    $resultat=$request -> fetchall();
                    for ($i=0; $i < count($resultat); $i++) { 
                      if($id==$resultat[$i][0]){
                        $request=$connexion -> prepare("DELETE FROM delegue where Matricule='$id'");
                        $request -> execute();
                        break;
                      }
                    }
            ...
            <?php
              ...
              try{
                ...
                if(isset($_POST['submit'])){
                  ...
                  $d=$_POST['pas'];
                  for ($i=0; $i < count($resultat); $i++) {
                    if(strcasecmp($b,$resultat[$i][0])==0){
                      $bool=0;
                      echo "<p style='color:red;'>Veuillez choisir un autre identifiant pour cet utilisateur.</p>";
                      break;
                    }
                  }
                  if($bool==1){
                    $insertion ="INSERT INTO delegue(Matricule,Nom,Niveau,Passwor,ID_chef)VALUES('$b','$c','$a','$d','123456')";
                ...
                echo "error";
                  /*$insertion = $connexion -> prepare("INSERT INTO delegue(Matricule,Nom,Niveau,Passwor,ID_chef)VALUES(:b,:c,:a,:d,'123456')");
                  $insertion -> bindParam(':b',$b);
                  $insertion -> bindParam(':c',$c);
                  $insertion -> bindParam(':a',$a);
                  $insertion -> bindParam(':d',$d);
                  $insertion -> execute();*/
              }
              $request -> execute();
              $resultat=$request -> fetchall();
